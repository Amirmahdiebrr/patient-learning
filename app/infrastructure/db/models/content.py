...
Hospital Override > Procedure-specific (EducationSection.procedure_id)
> Standard Department Content (department_type_id on
EducationSection, procedure_id NULL) > Global Content
(department_type_id NULL). See content_targeting_service.py.
...
patients in that specific hospital.
...
StandardDepartmentType (department_type_id) and, within that, to one
Procedure (procedure_id) - same override semantics as
EducationSection: procedure-specific quiz overrides department-general
quiz for the same stage+department, NULL means "shown to everyone at
that stage/department".
"""
...
class ContentTargetingRule(Base):
    ...
    (journey_stage + department_type + procedure) layer.
    """
